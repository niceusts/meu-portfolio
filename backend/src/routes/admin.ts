...
import { uploadToAzure } from '../lib/azure.js';
...
export async function adminRoutes(app: FastifyInstance) {
  ...
  app.put('/admin/skills/:id', async (req, reply) => {
    ...
    const { id } = req.params as { id: string };
    const body = req.body as any;
    const skill = await prisma.skill.update({ where: { id: Number(id) }, data: body });
  ...
  app.put('/admin/experiences/:id', async (req, reply) => {
    ...
    const { id } = req.params as { id: string };
    const body = req.body as any;
    const experience = await prisma.experience.update({ where: { id: Number(id) }, data: body });
  ...
  app.patch('/admin/messages/:id/read', async (req, reply) => {
    requireAdmin(req, secret);
    const { id } = req.params as { id: string };
    const message = await prisma.message.update({ where: { id: Number(id) }, data: { read: true } });
